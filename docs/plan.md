...
- ✅ **Cost tracking** (PR #3): `TurnLogEntry.usage` captures per-turn LLM spend from pi-mono's pi-ai usage payload, `lib/cost.ts` provides pure aggregation helpers (`sumUsage`, `costByLeClass`, `costByDay`, `costPerMasteredKc`), learning report renders a Cost section with by-LE-class breakdown and the cost-per-mastered-KC headline metric.
- ✅ **UI polish + HTML dashboard** (this PR): terminal report refactored to use shared `lib/report-data.ts` and hardened (proper section numbering, inline bar charts, ANSI color, no padding bugs). New `scripts/dashboard.ts` generates a single self-contained `report.html` with SVG learning curves, KPI strip, daily spend sparkline, per-topic mastery bars, and editorial data-journalism aesthetic (Fraunces serif + JetBrains Mono, warm dark palette, burnt-orange accent for regression signals). No build step, no server.
- 🔜 **Open follow-ups**: budget caps with soft-cancel, Bayesian Knowledge Tracing on the opportunity log, generative probes, cross-lingual interference detection, authoring tool, curves auto-promoting KCs to different LE classes when flat.
