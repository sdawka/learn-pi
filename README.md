...
- **Learning report (terminal)** — per-KC indicators, LE mix with inline bar charts, learning curves (avg error rate by opportunity, KLI's signature measurement), per-topic mastery, probe-vs-practice gap. Pure data extraction lives in `learn-pi/lib/report-data.ts` so both renderers consume the same source. — `learn-pi/scripts/learning-report.ts`
- **HTML dashboard** — single `report.html` with all data and CSS inlined, SVG learning curves, KPI strip, per-LE-class cost breakdown, daily spend sparkline, per-topic mastery bars, and the probe-vs-practice gap. Editorial data-journalism aesthetic (Fraunces serif headlines + JetBrains Mono data, warm near-black palette, burnt-orange accent for regression signals). No JS runtime, no server, no build step — open the file. (Fonts load from Google Fonts; offline falls back to Georgia + system mono.) — `learn-pi/scripts/dashboard.ts`
