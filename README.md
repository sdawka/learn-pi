...
- **HTML dashboard** — single self-contained `report.html` with SVG learning curves, KPI strip, per-LE-class cost breakdown, daily spend sparkline, per-topic mastery bars, and the probe-vs-practice gap. Editorial data-journalism aesthetic (Fraunces serif headlines + JetBrains Mono data, warm near-black palette, burnt-orange accent for regression signals). No build step, no server, no runtime dependencies — just open the file. — `learn-pi/scripts/dashboard.ts`
