...
pins the model to `google/gemini-3.1-flash-lite-preview` routed through
OpenRouter (see the script for why we can't persist this via pi's
saved-default settings), and execs `pi`. The extensions read the vault
path from pi's cwd — there is no `--vault` flag. For a different model,
