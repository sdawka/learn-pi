...
export default function learnLoop(pi: ExtensionAPI): void {
  ...
  pi.on("before_agent_start", async (event: { systemPrompt: string }, _ctx) => {
    ...
    const directive = [
      ...
      `The names below in quotes are SKILLS — markdown guides embedded in this system prompt, NOT tools. Read the skill's instructions and follow them; do not try to call a tool by that name.`,
      `1. Follow the "zpd-calibrate" skill to derive the style directive for this turn.`,
      `2. Call le_declare exactly once with one of memory_fluency | induction | sense_making. Match the dominant KC type the turn engages:`,
      `   - memory_fluency → fact retrieval (follow "recall-weave" to weave due items)`,
      ...
      `3. If you skip le_declare the turn is logged as "(undeclared)" and the report flags it.`,
      ``,
      `Confusion signals:`,
      `- A single "?" (exactly one question mark) → follow "spot-gloss": gloss only the confusing token in base language, leave the rest of the turn intact, do NOT change ZPD.`,
      `- "??" / "huh" / a second "?" within two turns → follow "simplify-ladder": drop ZPD subscore and rewrite the whole previous turn at the lower rung.`,
      ``,
      `Proactivity:`,
      `- If the user message is empty (proactive ticker), follow "proactive-opener": ground the opener in LEARNER.md + recent MEMORY.md entries.`,
      `- When the user reacts notably to something (lights up, gets frustrated, shares a detail), follow "memory-append" to record it.`,
      ``,
